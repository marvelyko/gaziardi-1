...
from myproject.models import User,Admin,Product,Category, Bid
...
def index():
    bids = Bid.query.all()
    return render_template("admin-index.html", bids = bids)
...
def add_products():
    form = AddProductForm()
    if form.validate_on_submit():
        product = Product(form.name.data,form.manufacturer.data,form.stock.data,100)
        ...
        product.min_bid = form.minBid.data
...
@admin_blueprint.route("/cancel/<int:id>")
@admin_required
def cancel(id):
    bid = Bid.query.get(id)
    bid.product.sold_quantity -= bid.quantity
    bid.status = "canceled"
    db.session.add(bid)
    db.session.commit()
    return redirect(url_for("admin.index"))

@admin_blueprint.route("/approve/<int:id>")
@admin_required
def approve(id):
    bid = Bid.query.get(id)
    bid.status = "moving"
